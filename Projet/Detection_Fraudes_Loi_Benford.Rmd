...
Notons $H_0$ l'hypothèse nulle.

![Tableau 2 : Règle de décision.](Images/erreurs.png)
...
Les tests du $\chi^2$  sont des tests d'hypothèses statistiques non-paramétriques. Ils comparent la distribution observée dans un échantillon statistique à une distribution théorique (*l'ajustement*), à tester si deux caractères d'une population sont indépendants (*l'indépendance*) et à tester si des échantillons sont issus d'une même population (*l'homogénéité*). Le test lit l'écart critique dans la table de la loi du chi-deux.  
...
Le déroulement du test se procède en 5 étapes:  
 ...
 4) Si $\chi^{2}_a < \chi^2$, on accepte l'hypothèse $H_0$ (la distribution observée est conforme à la distribution théorique" avec un risque d'erreur a) sinon on la rejette.  
 5) Il faut vérifier que $n_{pj} \geq 5$ pour tout j.  
![Tableau 3 : Densité du Chi-Deux en fonction du nombre de degrés de liberté.](Images/Chi-square.png)
...
où X admet alors une espérance et une variance
$E(X)=n$ et $V(X)=2 n$  

# Test de Freedman qui est la modification du test de Watson

Ce test est pour des distribution discrètes entre les distributions des premiers chiffres et la distribution de Benford pour affirmer si les données sont conformes à la loi de Benford.  
...
De plus grands écarts entres les fréquences conduisent à un plus grand $U^2$ ce qui rend le rejet plus probable. Ce test est reconnue comme plus performant que les autres tests, il est même recommandé par (LESPERANCE et collab. 2016) et (JOENSSEN 2014) .
...
G. R. DUCHARME, S. KACI, C. VOVOR-DASSU *Tests d'adéquations lisses pour la loi de Newcomb-Benford*

M. AUSLOOS, R. CERQUETI, T. A. MIR *Data science for assessing possible tax income manipulation: the case of Italy*
