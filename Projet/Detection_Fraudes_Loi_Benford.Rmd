...
\underline{Mots-clés:} Loi de Benford-Newcomb, 1$^{er}$ chiffre significatif, Test d'hypothèses , Risque d'erreur.
...
nbsp;
La famille des tests lisses introduite par Neyman$^1$ s'applique à des données autant discrètes que continues. Il est spécifique à la loi de probabilité en $H_0$.
...
![](Images/theoreme1.png)

Nous avons donc nos statistiques de test, les $T_K$ .Il faut maintenant exprimer les $h_k$. 
Dans la suite l’indice 0 dénote un opérateur probabiliste calculé sous $H_0$ : X suit f(·).

D'après Ducharmes & Collab. et le théorème suivant: 

![](Images/theoreme2.png)
Nous avons pour $0<k\leq5$, les $h_k$ suivants:
...
et la statistique de test $T_{\hat{K}}$ qui $\stackrel{L}{\longrightarrow} \chi_{1}^{2}$ sous $H_{0}$.

Dans la suite nous appliquerons les test $T_{\hat{K}}$ et $T_2$ qui d'après Ducharme & Collab sont les plus performants.
