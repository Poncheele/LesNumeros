...
Notons que l'astronome, mathématicien, économiste et statisticien canadien Simon Newcomb fournira, en $1881$, une première approche au principe statistique qui se fera appeler *Loi de Benford*.  
...
Nous retrouvons cette loi dans énormément de domaines comme les mathématiques, l'environnement, la finance, la physique. Et plus précisément nous pourrions le voir sur des données telles que la longueur des fleuves, la population des villes dans un pays, des déclarations de revenus.
...
Il apparaît clairement que la répartition des chiffres significatifs des $300$ premiers nombres de la suite de Fibonacci est semblable à celle de la loi de Benford.  
...
```{r nb_premier}
# nb_premier<-read.table("C:/Users/laulau/LesNumeros/Projet/nombre_premier.csv",sep="\t",header=T)
```
