...
[^n1]: Les données dites naturelles sont celles qui n'ont pas été influencées par la pensée de l'homme.

\newpage

# Genèse de la loi de Newcomb-Benford
...
Dans la suite, nous appliquerons les tests $T_{\hat{K}}$ et $T_2$  avec $K_{max} = 5$ qui d'après Ducharme & Collab. sont les plus performants.
...
Les données se présentent comme ci-après, chaque ligne correspond à une de nos expériences, les $9$ premières colonnes (sans compter la première qui correspond à l'expérience) représente la répartition du premier chiffre significatif. Les $4$ dernières colonnes quant à elles sont les $p$-values des différents tests. Une case est rouge si l'hypothèse nulle est rejetée au seuil de $5 \%$ d'erreur.
...
Pour terminer, tous les tests mènent à un rejet de l'hypothèse nulle "le jeu de donnée suit la loi de N-B" pour les jeux de données sur les populations des villes de France, le passage journalier de vélos et les nombres générés par les humains. 
...
Les données se présentent comme ci-après, chaque ligne correspond à une année, les $9$ premières colonnes (sans compter la première qui correspond à l'année) représente la répartition du premier chiffre significatif. Les $4$ dernières colonnes quant à elles sont les $p$-values des différents tests que nous avons cités plus haut. Une case est rouge si l'hypothèse nulle est rejetée au seuil de $5 \%$ d'erreur.
...
&nbsp;
La crise du COVID-$19$ étant au coeur des débats actuels et ayant un impact non négligeable sur notre scolarité, nous avons voulu nous intéresser aux chiffres que la Chine a publié. La Chine, à l'origine du départ de cette pandémie à publié des données sur l'expansion des cas dans son territoire.
...
&nbsp;
Au cours de ce projet, nous avons pu acquérir de nouvelles connaissances, ce fut intéressant et enrichissant de réaliser nos propres expériences et de nous initier au vaste problème de la détection de fraudes. Nous avons plus particulièrement découvert la loi de Newcomb-Benford, en exposant sa genèse, mais aussi en l'étudiant au travers de différents tests. De ce fait, nous avons tenté de répondre à certaines questions : quelle répartition suit le premier chiffre significatif d'une suite de nombres générés naturellement ? Les tests mis en place pour détecter une fraude sont-ils fiables et vont-ils toujours dans le même sens ? Certains sont-ils meilleurs que d'autres ?  
...
Une observation visuelle n'étant que peu efficace, nous avons réalisé différents tests. Notons que la réalisation d'un test n'est pas complètement fiable, et qu'il faut tenir compte des risques d'erreurs ; le risque de suspecter à tort une fraude (risque de première espèce) et le risque de 
...
De plus, lors des applications des tests, nous avons remarqué que certains tests avaient une $p$-value inférieurs à $5 \%$. Les tests paraissent fiables mais pour " détecter une fraude plus y a de test mieux c'est " Car les tests ne vont pas dans les mêmes directions car chaque test met en valeur leurs "propriétés " c'est- à-dire qu'un permet de ... et un autre de ..
(Ouverture)
