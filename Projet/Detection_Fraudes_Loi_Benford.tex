...
\(p\)-values des différents tests que nous avons cité plus haut. Une
...
attendus. Nous-mêmes nous sommes allées chercher les données de cette
...
de fraudes. Nous avons plus particulièrement découvert la loi de
Newcomb-Benford, en exposant sa genèse, mais aussi en l'étudiant au
travers de différents tests. De ce fait, nous avons tentés de répondre à
certaines questions : quelle répartition suit le premier chiffre
significatif d'une suite de nombres générés naturellement ? Les tests
mis en place pour détecter une fraude sont-ils fiables et vont-ils
toujours dans le même sens ? Certains sont-ils meilleurs que d'autres ?
...
Une observation visuelle n'étant que peu efficace, nous avons réalisés
...
le risque de suspecter à tort une fraude (risque de première espèce) et
le risque de
...
De plus, lors des applications des tests, nous avons remarqué que
certains tests avaient une \(p\)-value inférieurs à \(5 \%\). Les tests
paraissent fiables mais pour " détecter une fraude plus y a de test
mieux c'est " Car les tests ne vont pas dans les mêmes directions car
chaque tests mettent en valeurs leurs ``propriétés'' c'est- à-dire que
un permet de \ldots{} et un autre de .. (Ouverture) Pour avoir un autre
point de vue, il serait bien d'appliquer la loi de Newcomb-Benford avec
le deuxième chiffre significatif et lui appliquer les différents tests
pour permettre à une autre conclusion ?
